...
> **免终端提示（Windows）**：把 `ai.env.example` 复制为 `ai.env`（仓库根目录），
> 填好 KEY=VALUE 后双击 `run-game.bat` 即可，无需在终端敲环境变量。
> `serve.bat` 与 `tools/ai_proxy.py` 也读同一份 `ai.env`；终端里已设置的
> 环境变量优先于文件。`ai.env` 含密钥，已在 `.gitignore` 中，不会入库。
