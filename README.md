...
已验证：监听 → 接入占座 → ready → **开局** → 客户端收到 welcome/start/log。
**未验证**：通过 TCP 完整打完一局（目前跑到开局后会停住，见下）。
...
尚未做 / 有问题：
- **TCP 下完整对局未跑通** —— 服务端开局后推进会停住（进程内亦如此），
  怀疑是 Host 等人类应答与服务端 drain/tick 的时序问题，待排查。
  内存通道下同样的 Host 逻辑能完整跑完（27 项网络测试通过），
  所以问题在 TCP 适配层而非房间/座位模型。
- UI 联调（真实客户端替掉本地人类玩家）、断线重连、观战、聊天。
