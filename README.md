...
D（网络对局）进行中：联机/重连/观战/聊天与 UI 联调已通，缺多房间大厅。
F（LLM 驱动的 AI 玩家）链路完成，单机与联机（`SGS_NET_AI`）都已接入。
...
测试：**核心 443 + UI 68 + AI 107 = 618 全通过**（另有网络 51，走 `--net`）。
...
| F | LLM 驱动的 AI 玩家 | ✅ 完成：mock 全链路 107 项测试 + TokenHub hy3 真机实测（关思维链后一局约 6 分钟）；观星重排、联机 AI 已接入 |
